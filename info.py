...
BOT_TOKEN = environ['BOT_TOKEN', "7661519627:AAHUAJ_JvTLiggdzec11ORSEt9NIcL4jMio"]
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7216492043 7314923110').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002293883284').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '7216492043 7314923110').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = [int(auth_channel) for auth_channel in environ.get('AUTH_CHANNEL', '-1002162032884').split() if id_pattern.search(auth_channel)]
...
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://darkpanda:darkpanda@darkpanda.he3ux.mongodb.net/?retryWrites=true&w=majority&appName=darkpanda")
DATABASE_NAME = environ.get('DATABASE_NAME', "darkpanda")
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', "-1002450813563"))
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', "-1002293883284")).split()]
